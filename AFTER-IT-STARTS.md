...
Answering one question stops three parts of the Settings screen guessing about
your deployment. Go to **Settings**, then the **Network** tab. The first section
is **How this system is reached**, and it offers three answers:
...
**Answer it even though nothing appears to depend on it.** Until you do, the
encryption section, the ports section and the advice about certificates each work
it out from your address alone, so PathLMS may offer to make you a certificate
you already have. Answering once stops that.
...
sign-in page renders perfectly and then does nothing, uploaded pictures do not
appear, and no error on the screen points at the cause. It is worth thirty
seconds of your own eyes.
...
`PATHLMS_PUBLIC_URL` in your settings file. Four things derive from it:
...
Backups run on their own from the first start, so nobody has to arrange one. The
database and the uploaded files are copied once a day by default, kept for
...
This is the version to run today, while nothing is wrong. Make the scratch
database first, because the script restores into a database and does not create
one:
...
the most recent copy in it. Naming a different `DB_NAME` puts the restored copy
...
**It asks you to type the word `RESTORE` either way**, because it replaces
whichever database it was pointed at. Read the two lines above that prompt before
you answer: they name the file and the database, and that is your last chance to
notice you are about to overwrite the live one.
